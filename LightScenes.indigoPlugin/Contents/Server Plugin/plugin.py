...
class Plugin(indigo.PluginBase):
    ...
    def startup(self):
        ...
        indigo.server.log("Running plugin version check (if enabled).")
        ...
        if not self.pluginPrefs.get('scenesList', False) or len(self.pluginPrefs['scenesList']) == 0:
            ...
            for theScene in scenes:
                ...
                indigo.server.log("pluginPrefs checking scene=" + theScene[0])
                if len(theScene) > 3:
                    colorDict = dict()
        ...
        if 'scenes' in self.pluginPrefs:
            indigo.server.log("removing old 'scenes' from pluginPrefs.")
                                    ...
                                    if sceneThreadActive is True:
                                        if workItem.isGroupWork is False:
                                            ...
                                            if workItem.workItemType is WorkItemType.RGB:
                                                ...
                                                indigo.server.log("\"%s\" %s to %s" % (indigo.devices[workItem.theDevice.id].name, "set color", str(workItem.colorTemp)))
                                                ...
                                                indigo.server.log("\"%s\" %s to %s" % (indigo.devices[workItem.theDevice.id].name, "set brightness", str(workItem.brightness)))
    ...
    def startScene(self, action, device):
        self.debugLog("startScene: device.id: " + str(device.id) + ", action:\n" + str(action))
    ...
    def validateRGBWConfig(self, configStr, valuesDict, errorsDict):
        ...
        if isTemp == True and (colorTemp < 1500 or colorTemp > 6500):
            errorsDict[configStr] =  configStr + " ColorTemp must be between 1500 and 2500."
            errorsDict['showAlertText'] += errorsDict[configStr]
            errorText = configStr + " ColorTemp must be between 1500 and 2500."
            self.errorLog(errorText)
            return False, valuesDict, errorsDict
    ...
    def validateCircadianConfig(self, configStr, valuesDict, errorsDict):
        ...
        if isTemp is True and (lateNight < 1500 or lateNight > 6500 or preSunrise < 1500 or preSunrise > 6500 \
            ...
            errorsDict[configStr] =  configStr + " values must be between 1500 and 6500."
            errorsDict['showAlertText'] += errorsDict[configStr]
            errorText = configStr + " values must be between 1500 and 6500."
            self.errorLog(errorText)
            return False, valuesDict, errorsDict
